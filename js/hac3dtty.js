import { ReturnErr_ } from "./libs/errorHandler.js";
var ttty = function(f) {
    "use strict";
    const c = (t, n, e) => {
    ...
    const m = (t, n, e) => {
        ...
        v = (t, n) => {
            ...
            i ? i.argDescriptions && i.argDescriptions.length > 0 && a.length === 0 ? e(`Usage: ${o} ${i.argDescriptions.map(u=>`[${u}]`).join(" ")}`) : (i.func(n, ...a), m(p.ON_COMMAND, n.settings.host, t)) : (e(ReturnErr_("NotFound", `'${o}' isn't a valid command.`)), m(p.ON_COMMAND_NOT_FOUND, n.settings.host, t))
        },
        _ = t => {
            const {
                settings: {
        ...
        b = (t, n) => {
            ...
            t.addEventListener("keyup", ({
                ...
                if (r && s === "c" && n.isProcessRunning) o(`^C<br>Focus removed. Clear terminal to close program.`), y(n), m(p.ON_PROCESS_INTERRUPT, t);
                else {
        ...
        E = t => ({
            ...
            func: ({ print: n }) => {
                n("<br>");
                for (const e in t.settings.commands) {
                    if (!Object.hasOwnProperty.call(t.settings.commands, e)) continue;
                    
                    const cmd = t.settings.commands[e];
                    if (cmd.noIndex) continue; // ⬅️ Skip commands with noIndex: true

                    n(`${cmd.name} <span style="position: absolute; left: 135px;">${cmd.description}</span>`);
                }
                n("<br>");
            }
        }),
